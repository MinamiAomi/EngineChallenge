...
void PipelineBase::SetPipelineState(ID3D12GraphicsCommandList* cmdlist, BlendMode blendmode) 
{
	cmdlist->SetPipelineState(m_pipelineState[blendmode].Get());
	cmdlist->SetGraphicsRootSignature(m_rootSignature[blendmode].Get());
}

void PipelineBase::SetBlendState(D3D12_RENDER_TARGET_BLEND_DESC& blendDesc, BlendMode blendmode) 
{
