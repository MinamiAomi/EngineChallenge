...
public: // 静的メンバ関数
	...
	static void Draw(Triangle2D& sprite, const class Camera2D* camera, BlendMode blend);
...
public:
	Shape2D();
	Shape2D(UINT  textureHandle, Vector2 position, Vector2 size, Vector4 color = { 1,1,1,1 },
		Vector2 anchorPoint = { 0,0 }, bool isFlipX = false, bool isFlipY = false);
